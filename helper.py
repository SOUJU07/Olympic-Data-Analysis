...
def fetch_medal_tally(df, years, country):
    ...
    if years != 'Overall' and country == 'Overall':
        temp_df = medal_tally[medal_tally['Year'] == int(years)]
    if years != 'Overall' and country != 'Overall':
        temp_df = medal_tally[(medal_tally['Year'] == int(years)) & (medal_tally['region'] == country)]
...
def most_successfull(df, sport):
    ...
    x = temp_df['Name'].value_counts().reset_index().head(15).merge(temp_df, left_on='index', right_on='Name', how='left')[
...
def country_wise_sport_medal(df,country):
    temp_df = df.dropna(subset=['Medal'])
    temp_df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'], inplace=True)
    new_df = temp_df[temp_df['region'] == country]
...
def most_successfull_player(df, country):
    temp_df = df.dropna(subset=['Medal'])

    temp_df = temp_df[temp_df['region'] == country]
    x = \
    temp_df['Name'].value_counts().reset_index().head(10).merge(temp_df, left_on='index', right_on='Name', how='left')[
